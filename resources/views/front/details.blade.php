@extends('layouts.app')
@section('title') Gym di {{ $gym->name }} Details @endsection
...
<main id="content" class="relative flex w-full max-w-[1280px] gap-6 mx-auto px-10 mt-[96px]">
    <section id="details" class="flex flex-col gap-6 w-full max-w-[820px] flex-1">
            ...
            <div class="flex items-center gap-8">
                <div class="flex items-center gap-3 shrink-0">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <p class="text-sm leading-19 tracking-05 opacity-50 text-nowrap">{{$gym->open_time_at->format('H:i A')}} - {{$gym->close_time_at->format('H:i A')}}</p>
                    </div>
                </div>
                <div class="flex items-center gap-8">
        ...
        <div id="reviews" class="flex flex-col w-full rounded-3xl p-8 gap-8 bg-white">
            ...
            <div class="grid grid-cols-2 gap-4 px-[27px]">
              @forelse ( $gym->gymTestimonials as $itemReview )
              <div class="font-['Poppins'] flex flex-col w-full rounded-3xl border border-[#E4E4E4] py-4 px-6 gap-3 bg-white">
                  <div class="flex items-center gap-3">
                      <div class="w-12 h-12 rounded-full border-[5px] border-white overflow-hidden">
                          <img src="{{Storage::url($itemReview->photo)}}" class="w-full h-full object-cover" alt="photo">
                      </div>
                      <div class="flex flex-col gap-1">
                          <p class="font-semibold text-sm leading-[22px] tracking-03">{{$itemReview->name}}</p>
                          <p class="text-xs leading-5 tracking-03 text-[#8D9397]">{{$itemReview->accupation}}</p>
                      </div>
                  </div>
                  <p class="text-sm leading-[22px] tracking-03">{{$itemReview->message}}</p>
    ...
    <aside class="flex flex-col gap-6">
        <div class="flex flex-col w-full rounded-3xl p-8 gap-6 bg-white">
            ...
            <a href="subscription-list.html" class="rounded-full py-3 px-6 bg-[#606DE5] font-semibold leading-19 tracking-05 text-white text-center">Become Member</a>
        </div>
